...
Each repository has a self-contained `pnpm dev` command that starts only that project's stack, not the entire ecosystem.
...
**Decision**: Each repo's `pnpm dev` (or `make dev`) starts:
